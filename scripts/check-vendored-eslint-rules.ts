 ...
 *
 * This repo resolves no dependency on `obsidian-dev-utils` - it is a solutions repo, not a consumer - so the
 ...
 *
 * This runs from `nano-staged` on a staged rule source, and by hand as `npm run check:vendored-eslint-rules`.
 * This repo has no CI workflow, so the drift direction only CI catches - upstream moving underneath a tree
 * nobody touches, which a hook never sees because nothing is staged - is caught here only by a hand run.
 *
 * The copies in the sibling repos carry a per-script env toggle here, so a run with no network can opt out
 * explicitly rather than be skipped silently. This repo has no `helpers/env-toggle.ts` to hang one on, so the
 * toggle is left out rather than ported in for this one script: run the script or do not.
 */
